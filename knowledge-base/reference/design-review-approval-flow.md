...
5. **Agent review**: The Foundry agent reads the complete design, visually inspects every supplied PNG, compares each diagram with the written design, and evaluates the package using the repository instructions and checklists.
...
- Implement the Function HTTP trigger and event validation.
