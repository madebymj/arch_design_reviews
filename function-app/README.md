...
The Function atomically claims each Azure DevOps event ID in Blob Storage before starting the review. Azure DevOps retries for an active or completed event return HTTP `200` with `status: ignored`, preventing duplicate agent runs and Board comments. The claim uses the existing `AzureWebJobsStorage` account and the `REVIEW_IDEMPOTENCY_CONTAINER` setting, which defaults to `architecture-review-events`. Claims older than ten minutes can be recovered after a worker termination. A failure before Board writeback releases its claim so Azure DevOps can retry it; a successful review retains a completed claim. An uncertain Board write failure retains a failure marker to avoid a second comment.
